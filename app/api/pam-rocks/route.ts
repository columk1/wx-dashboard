...
const PAM_ROCKS_URL = 'https://weather.gc.ca/past_conditions/index_e.html?station=was'
...
const withUpdatedAtText = (data: WXCardData, observedAt: number): WXCardData => {
  if (!data) return data
...
const parsePamRocksObservation = (html: string): PamRocksObservation | null => {
  const tableMatch = html.match(/<table[^>]*id="past24Table"[\s\S]*?<tbody>([\s\S]*?)<\/tbody>/i)
  ...
  for (const [, rowHtml] of rowMatches) {
    ...
    const cells = [...rowHtml.matchAll(/<td[^>]*>([\s\S]*?)<\/td>/gi)].map(([, cell]) => cell)
    ...
    const observedAt = fromZonedTime(
      parse(`${currentDateLabel} ${timeLabel}`, 'd MMMM yyyy HH:mm', new Date()),
      PACIFIC_TIMEZONE
    ).getTime()
...
const getCachedPamRocksData = unstable_cache(fetchPamRocksData, ['pam-rocks-data-store-v2'], {
  tags: [PAM_ROCKS_TAG],
})
...
export async function GET() {
  try {
    ...
    if (!observation?.data) {
      return NextResponse.json({ error: 'Failed to fetch Pam Rocks data' }, { status: 500 })
    }
    ...
    return NextResponse.json(withUpdatedAtText(withoutGusts(observation.data), observation.observedAt))
  } catch (error) {
